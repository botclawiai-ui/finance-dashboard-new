...
export async function fetchDashboardData(accountNames?: string[], timeframe: Timeframe = "last6Months") {
    try {
        ...
        if (accountNames && accountNames.length > 0) {
            transactions = transactions.filter(tx => accountNames.includes(tx.account));
        }
        ...
        // Fetch Accounts for real balance
        const accountsQuery = query(collection(db, "accounts"));
        const accountsSnapshot = await getDocs(accountsQuery);
        let realBalance = 0;
        let useRealBalance = false;

        accountsSnapshot.forEach(doc => {
            const accDoc = doc.data();
            if (!accountNames || accountNames.length === 0 || accountNames.includes(accDoc.name)) {
                // If accounts exist, we sum their actual balances.
                realBalance += accDoc.balance || 0;
                useRealBalance = true;
            }
        });
        ...
        return {
            transactions: transactions.slice(0, 100), // Grab latest 100 for better local filtering
            monthlyData,
            expensesByCategory,
            summary: {
                totalIncomeMTD: totalIncome, // Reuse key for compatibility
                totalExpensesMTD: totalExpenses,
                netSavingsMTD: totalIncome - totalExpenses,
                netWorth: mockSummary.investmentTotal + totalIncome - totalExpenses, // Simple mock calc
